...
class BankController extends Controller
{
    ...
    public function getBankByUserID()
    {
        try {
            ...
            $BankUser = UserBank::with('users.userDetail')->where('user_id', $user_id)->get();
    ...
    public function getBankUtamaByUserID()
    {
        try {
            //code...
            $user_id = Auth::user()->id;
            $BankUser = UserBank::with('users.userDetail')->where('user_id', $user_id)->where('rekening_utama', true)->get();
            return response()->json(['data' => $BankUser, 'message' => 'Success'], 200);
        } catch (\Throwable $th) {
            //throw $th;
            return response()->json(['message' => 'Internal Server Error'], 500);
